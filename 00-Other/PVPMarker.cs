...
[ScriptType(name: "PVPMarker", territorys: [431, 554, 888, 791], guid: "7be8cf60-b58a-42e4-abe1-11202b3f3f4e",
...
public class PVPMarker
{
    ...
    public async void Markers(Event @event, ScriptAccessory accessory)
    {
        ...
        if (isText) accessory.Method.TextInfo($"击杀者: {@event.SourceName()}, 职业: {IbcHelper.GetById(@event.SourceId()).ClassJob.GameData.Name}", duration: 6000, true);
        accessory.Method.SendChat($"/e 击杀者: {@event.SourceName()}, 职业: {IbcHelper.GetById(@event.SourceId()).ClassJob.GameData.Name}");
        accessory.TTS($"/e 击杀者: {@event.SourceName()}, 职业: {IbcHelper.GetById(@event.SourceId()).ClassJob.GameData.Name}", isTTS, isDRTTS);
        await Task.Delay(50);
    ...
    public async void debugge(Event @event, ScriptAccessory accessory)
    {
        ...
        DebugMsg($"job:{IbcHelper.GetMe().ClassJob.GameData.Name}", accessory);
    }
